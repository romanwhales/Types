...
let halfShare:any = taxAmount / 2;
console.log(`Full Amount in tax ${taxAmount}`);
console.log(`Half Share: ${halfShare}`);

function calculateTaxFormat(amount:number,format:boolean):string | number | null{
    if(amount === 0){
        return null;
    }
...
let taxNumber2 = calculateTaxFormat(100,false) as number;
let taxString2 = calculateTaxFormat(100,true) as string;
...
let taxBoolean = calculateTaxFormat(100,false) as any as boolean;
console.log(`Boolean Value: ${taxBoolean}`);

let taxValue:string|number|null = calculateTaxFormat(100,false);
...
let taxValue2: string | number|null = calculateTaxFormat(0,false);
...
switch(typeof taxValue2){
    ...
    default:
        if(taxValue2 === null){
            ...
            let value: never = taxValue2;
            console.log(`Unexpected type for value: ${value}`)
        }
        
}

let newResult2:unknown = calculateTaxFormat(200,false);
let myNumber2: number = newResult2 as number;
console.log(`Number Value: ${myNumber2.toFixed(2)}`)

let taxValue3: string| number | null = calculateTaxFormat(100,false); 
if(taxValue3 !== null){
    let nonNullTaxValue:string | number = taxValue3;
...
let taxValue4! :string | number| null;
...
function calculateTaxWithRest(amount:number|null,discount:number=0,...extraFees:number[]){
    if(amount != null){
...
function calculateTaxReturnNumber(amount:number,discount:number=0,...extraFees:number[]):number{
    return (amount*1.2) - discount + extraFees.reduce((total,val) => total + val ,0);
}
...
function calculateTax6(amount:number|null):number|null{
    if(amount != null){
        return amount *1.2;
    }
    return null;
}
let taxAmount6:number | null = calculateTax6(100);
if(typeof taxAmount6 === "number"){
    writeValue("Tax Value6 ",taxAmount6);
}

console.log("Chapter 9");
function calculateTax9(amount:number):number{
    return amount * 1.2;
}
...
let prices9: number[] = [100,75,42];
let names9:string[] =["Hat","Gloves","Umbrella"];
...
prices9.forEach((price:number,index:number) => {
